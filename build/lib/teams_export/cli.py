...
from .exporter import ChatNotFoundError, choose_chat, export_chat, export_chat_incremental
...
def main(
    ...
    download_all_attachments: bool = typer.Option(
        False,
        "--download-all-attachments/--download-images-only",
        help="Download ALL attachment types (PDFs, docs, etc), not just images (default: images only).",
    ),
    incremental: bool = typer.Option(
        False,
        "--incremental",
        help="Use incremental sync to only fetch new messages since last export.",
    ),
    parallel_fetch: bool = typer.Option(
        True,
        "--parallel-fetch/--sequential-fetch",
        help="Fetch chat pages concurrently for faster exports (disable if you encounter hangs or throttling).",
    ),
) -> None:
    try:
        config = load_config()
    ...
    with GraphClient(token) as client:
        ...
        # Initialize delta manager if using incremental mode
        delta_manager = DeltaStateManager() if incremental else None
                ...
                try:
                    if incremental:
                        output_path, count, has_changes = export_chat_incremental(
                            ...
                            download_all_types=download_all_attachments,
                            delta_manager=delta_manager,
                            use_parallel_fetch=parallel_fetch,
                        )
                        if not has_changes:
                            return (title, output_path, count, "No new messages")
                    else:
                ...
                for future in as_completed(futures):
                    ...
                    if error:
                        if error == "No new messages":
                            typer.secho(f"[{completed}/{len(selected_chats)}] No new messages: {title}", fg=typer.colors.YELLOW)
                        else:
            ...
            for chat in selected_chats:
                ...
                try:
                    if incremental:
                        ...
                        if not has_changes:
                            typer.secho(f"No new messages since last sync for {title}", fg=typer.colors.YELLOW)
                            continue
                    else:
                        output_path, count = export_chat(
                            client,
