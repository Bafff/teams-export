...
def write_html(
    ...
    html_parts = [
        ...
        '    </style>',
        '    <script>',
        '        // Store image data globally for clipboard operations',
        ...
        '        }',
        '',
        '        // On page load, extract and cache all base64 images',
        '        window.addEventListener("DOMContentLoaded", async () => {',
        '            const images = document.querySelectorAll("img[src^=\'data:\']");',
        '            for (let img of images) {',
        '                try {',
        '                    const blob = await base64ToBlob(img.src);',
        '                    const imageId = `img_${imageCache.size}`;',
        '                    imageCache.set(imageId, blob);',
        ...
        '',
        '                // Use execCommand which properly handles rendered images',
        ...
        '                    status.style.backgroundColor = "#107c10";',
        ...
        '                status.textContent = "❌ Copy failed. Try manually: Ctrl+A then Ctrl+C";',
        ...
        '        }',
        '',
        '        // Intercept copy event to ensure images are included as rendered',
        '        document.addEventListener("copy", (e) => {',
        '            const selection = window.getSelection();',
        '            if (!selection.rangeCount) return;',
        '',
        '            // Let the browser handle the copy naturally for rendered content',
        '            // This should include base64 images as rendered images',
        '        });',
        '    </script>',
        '</head>',
        '<body>',
